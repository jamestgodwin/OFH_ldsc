...
class RG(object):
    ...
    def __init__(self, z1, z2, x, w, N1, N2, M, intercept_hsq1=None, intercept_hsq2=None,
        ...
        if (hsq1.tot <= 0 or hsq2.tot <= 0):
            ...
            self.rg_jknife = float(np.asarray(rg.jknife_est).flatten()[0])
            self.rg_se = float(np.asarray(rg.jknife_se).flatten()[0])
            self.rg_ratio = float(np.asarray(rg_ratio).flatten()[0])
